...
export default function SetupPage() {
  ...
  const [webhookUrl, setWebhookUrl] = useState('https://3ba14692f5ed.ngrok-free.app/webhook')
  ...
  const {
    ...
    defaultValues: {
      config: {
        ...
        verify_token: '',
        waba_id: '',
        webhook_url: 'https://3ba14692f5ed.ngrok-free.app/webhook',
      },
  ...
  const onSubmit = async (data: PlatformConfig) => {
    try {
      ...
      const payload = {
        ...data,
        department_id: 3, // Updated to match your working example
        tenant_id: 27,    // Updated to match your working example
      }
  ...
  const handleDelete = async (platform: string) => {
    ...
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_API_BASE_URL}/platform-config/${platform}`, {
        params: { tenant_id: 27, department_id: 3 }
      })
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                {editingConfig ? 'Edit Platform Configuration' : 'Create New Platform Configuration'}
              </CardTitle>
              <CardDescription>
                          ...
                          <div className="space-y-3 flex-1">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                              ...
                              <div>
                                ...
                                <span className="ml-2 font-mono text-xs break-all">{config.config.webhook_url || 'N/A'}</span>
                              </div>
                              <div>
                                <span className="font-medium text-gray-700 dark:text-gray-300">Department:</span>
                                <span className="ml-2">{config.department_id}</span>
                              </div>
                              <div>
                                <span className="font-medium text-gray-700 dark:text-gray-300">Tenant:</span>
                                <span className="ml-2">{config.tenant_id}</span>
                              </div>
                            </div>
